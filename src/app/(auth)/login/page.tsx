...
import { Shield, Mail, Lock, ArrowRight, Eye, EyeOff, Sparkles, CheckCircle, AlertCircle, User, Loader2 } from 'lucide-react';
import { useToast } from '@/components/Toast';
import { useAuth } from '@/lib/auth-context';
import { landing, type LandingLocale } from '@/lib/landing/translations';

// ─── Read locale from localStorage (same key the landing page sets) ───────────
function useLandingLocale(): LandingLocale {
  ...
  useEffect(() => {
    try {
      ...
      if (stored && landing[stored]) setLocale(stored);
    } catch {}
  }, []);
  return locale;
}
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (authMode === 'signup') {
      ...
      if (err) {
        ...
        if (err2) { setError('Unable to create account. Please try again.'); setSubmitting(false); return; }
        if (email.endsWith('@demo.com')) {
          router.push(email.includes('client') ? '/portal/dashboard' : '/dashboard');
        } else { setMagicSent(true); setSubmitting(false); }
        return;
      }
      fetch('/api/auth/send-welcome', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: name || email.split('@')[0], accountType: 'individual' }),
      }).catch(() => {});
      router.push(email.includes('client') ? '/portal/dashboard' : '/dashboard');
      return;
    }
    ...
    if (err) { setError('Incorrect email or password. Try again.'); setSubmitting(false); return; }
    router.push(email.includes('client') ? '/portal/dashboard' : '/dashboard');
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-slate-950">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 w-full max-w-md text-center shadow-2xl">
          <div className="w-14 h-14 rounded-2xl bg-red-600/10 border border-red-600/20 flex items-center justify-center mx-auto mb-4">
    ...
    <div className="min-h-screen flex bg-slate-950">
      ...
      <div className="w-full lg:w-[48%] flex items-center justify-center p-6 lg:p-12"
        ...
        <div className="w-full max-w-md">
          ...
          {fromOnboarding && (
            <div className="rounded-xl p-3.5 mb-5 flex items-start gap-2.5 bg-red-950/40 border border-red-900/40">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Your details are filled in. {authMode === 'signup' ? 'Set a password to protect your case.' : 'Sign in to access your case portal.'}
                </p>
              </div>
            </div>
          )}
          ...
          {/* Error */}
          {error && (
            <div className="flex items-start gap-2.5 bg-red-950/60 border border-red-800 rounded-xl p-3 mb-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {authMode === 'signup' && (
              <div>
                <label className="block text-xs text-slate-500 mb-1.5 font-black uppercase tracking-wide">Name</label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                  <input type="text" value={name} onChange={e => setName(e.target.value)}
                    placeholder={t.namePh} className={inputCls + ' pl-9'} autoComplete="name" />
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  placeholder={t.emailPh} className={inputCls + ' pl-9'} required autoComplete="email" />
              </div>
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPass ? 'text' : 'password'} value={password}
                    ...
                    placeholder={t.passPh} className={inputCls + ' pl-9 pr-9'} autoComplete="current-password" />
            ...
            <button type="submit" disabled={submitting}
              className="w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-500 disabled:opacity-60 text-white font-black py-3 rounded-xl transition-all shadow-lg shadow-red-600/25 mt-1">
              {submitting
                ...
                {forgotSent ? (
                  <div className="text-center py-4">
                    ...
                    <button onClick={() => { setForgotMode(false); setForgotSent(false); setForgotEmail(''); }}
                      className="text-xs text-red-500 hover:text-red-400 font-bold">{t.backToSignIn}</button>
                  </div>
                ) : (
                  <>
                    ...
                    <form onSubmit={handleForgotPassword} className="space-y-3">
                      <div className="relative">
                        ...
                        <input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)}
                          placeholder={t.emailPh} className={inputCls + ' pl-9'} required autoFocus />
                      </div>
                      <button type="submit" disabled={forgotLoading || !forgotEmail.trim()}
                        ...
                        {forgotLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending…</> : t.sendReset}
                      </button>
          ...
          <div>
            ...
            <div className="space-y-2.5">
              {t.panel.watchList.map((item, i) => (
                <div key={i} className={`flex items-center gap-3 text-sm font-semibold rounded-xl px-4 py-3 ${
                  dark
