...
export default function CasePage() {
  ...
  const tasks     = mockTasks.filter(tk => tk.case_id === caseData.id);
          ...
          <div className="space-y-2 max-w-2xl">
            {tasks.map(task => (
              <div key={task.id} className="bg-white border border-stone-200 rounded-xl flex items-start gap-3 p-4 shadow-sm">
                <div className={cn('w-5 h-5 rounded flex items-center justify-center mt-0.5 flex-shrink-0 border-2',
                  task.status === 'done' ? 'bg-green-500 border-green-500' : 'border-stone-300')}>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    <span className="text-xs text-stone-400">{t.caseDetail.backToCases.includes('Back') ? 'Due' : t.shared.dueDate} {formatDate(task.due_date)}</span>
                    <span className="text-stone-300">·</span>
                    <span className="text-xs text-stone-400">{task.assignee_name}</span>
                  </div>
                </div>
                <select defaultValue={task.status}
                  className="text-xs border border-stone-200 rounded-lg px-2 py-1.5 bg-white text-stone-600 focus:outline-none focus:border-amber-400 cursor-pointer">
